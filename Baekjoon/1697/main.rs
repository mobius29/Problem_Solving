...
fn bfs(src: usize, dest: usize) -> usize {
    let mut deque: VecDeque<(usize, usize)> = VecDeque::new();
    deque.push_back((src, 0));

    let mut visited = [false; 2_000_001];
    visited[src] = true;

    while !deque.is_empty() {
        let (current_pos, current_count) = deque.pop_front().unwrap();
        if current_pos == dest {
        ...
        if current_pos != 0 {
            ...
            if !visited[next_pos] {
                ...
                deque.push_back((next_pos, next_count));
            }
        }

...
fn main() {
    ...
    let mut position_list: Vec<i32> = vec![-1; 26];
    let str: &str = line.next_str();

    str.chars().into_iter().enumerate().for_each(|(i, c)| {
        let ascii = (c as u8) - ('a' as u8);
        if position_list[ascii as usize] != -1 {
            return;
        }

        position_list[ascii as usize] = i as i32;
    });

    for i in 0..26 {
        write!(buf_write, "{} ", position_list[i]).ok();
    }

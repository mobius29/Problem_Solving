...
fn main() {
    ...
    loop {
        let triangle: Vec<usize> = line.next_iter().take(3).collect();
        if triangle.iter().all(|length| *length == 0) {
            break;
        }

        let mut squared: Vec<usize> = triangle.iter().map(|length| length * length).collect();
        squared.sort();

        let answer = if squared[0] + squared[1] == squared[2] {
            "right"
        } else {
            "wrong"
        };

...
impl<T> Queue<T> {
    ...
    pub fn back(&self) -> Result<&T, &str> {
        match self.back {
            Some(back_ptr) => {
                let return_value = unsafe { &(*back_ptr.as_ptr()).value };
                Ok(return_value)
            }
            None => Err("queue is empty!"),
        }
...
fn main() {
    ...
    for _i in 0..n {
        ...
        if instruction == "size" {
            writeln!(buf_write, "{}", queue.size).ok();
        }

        if instruction == "empty" {
            let answer = if queue.empty() { 1 } else { 0 };
            writeln!(buf_write, "{}", answer).ok();
        }

...
fn main() {
    ...
    let n: u32 = line.next();
    let scores: Vec<u32> = line.next_iter().take(n as usize).collect();

    let sum: u32 = scores.iter().sum();
    let max_score = scores.iter().max().unwrap();

    let new_sum = (sum as f32) / (*max_score as f32) * 100f32;
    let new_average = new_sum / (n as f32);

    writeln!(buf_write, "{}", new_average).ok();

...
fn _dfs(graph: &Vec<Vec<usize>>, current: usize, visited: &mut Vec<bool>, result: &mut Vec<usize>) {
    result.push(current);
...
fn dfs(graph: &Vec<Vec<usize>>, start: usize) -> Vec<usize> {
    ...
    visited[start] = true;
    _dfs(graph, start, &mut visited, &mut result);
    result
}
...
fn main() {
    ...
    let (n, m, v): (usize, usize, usize) = line.next();
    ...
    for idx in dfs(&graph, v) {
        write!(buf_write, "{} ", idx).ok();
    }

    writeln!(buf_write, "").ok();

    for idx in bfs(&graph, v) {
        write!(buf_write, "{} ", idx).ok();
    }

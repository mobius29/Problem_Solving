...
fn main() {
    ...
    let mut array: Vec<i32> = line.next_iter().take(n).collect();
    array.sort();

    let m: usize = line.next();
    let test_cases: Vec<i32> = line.next_iter().take(m).collect();

    test_cases.iter().for_each(|num| {
        let _ = match array.binary_search(num) {
            Ok(_) => writeln!(buf_write, "1"),
            Err(_) => writeln!(buf_write, "0"),
        };
    });

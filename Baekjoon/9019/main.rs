...
impl Register {
    ...
    fn operator_d(&self, cur: usize) -> usize {
        cur * 2 % 10000
    }

    fn operator_s(&self, cur: usize) -> usize {
        (cur + 9999) % 10000
    }

    fn operator_l(&self, cur: usize) -> usize {
        cur * 10 % 10000 + cur / 1000
    }

    fn operator_r(&self, cur: usize) -> usize {
        cur % 10 * 1000 + cur / 10
    }

    fn operation(&self, cur: usize, operator: char) -> Option<usize> {
        let next = match operator {
            'D' => self.operator_d(cur),
            'S' => self.operator_s(cur),
            'L' => self.operator_l(cur),
            'R' => self.operator_r(cur),
            _ => return None,
        };
    ...
    pub fn solve(&self) -> String {
        ...
        let mut is_visited = [false; 10000];

        queue.push_back((self.src, "".to_string()));
        is_visited[self.src] = true;

        while !queue.is_empty() {
            let (cur, operators) = queue.pop_front().unwrap();

            for c in ['D', 'S', 'L', 'R'].into_iter() {
                match self.operation(cur, c) {
                    Some(v) if !is_visited[v] => {
...
fn main() {
    ...
    for _ in 0..t {
        let (src, dest): (usize, usize) = line.next();
        let register = Register::new(src, dest);
        let answer = register.solve();

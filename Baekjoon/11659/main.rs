...
fn main() {
    ...
    let numbers: Vec<usize> = line.next_iter().take(n).collect();

    let prefix_sum: Vec<usize> = numbers.into_iter().fold(vec![0], |mut acc, cur| {
        let last_sum = acc[acc.len() - 1];
        acc.push(last_sum + cur);
        acc
    });

    for _ in 0..m {
        let (a, b): (usize, usize) = line.next();
        let answer = prefix_sum[b] - prefix_sum[a - 1];

...
fn main() {
    ...
    let mut difficulties: Vec<usize> = line.next_iter().take(n).collect();
    difficulties.sort();

    let cut_off = ((n as f32) * 0.15).round() as usize;
    let cut_off_difficulties = &difficulties[cut_off..(n - cut_off)];

    let sum: usize = cut_off_difficulties.iter().sum();
    let average: usize = (sum as f32 / (n - 2 * cut_off) as f32).round() as usize;

    writeln!(buf_write, "{}", average).ok();

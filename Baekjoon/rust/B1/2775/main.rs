...
fn main() {
    ...
    for _test_case in 0..t {
        let (k, n): (usize, usize) = line.next();

        let mut people_floors: Vec<Vec<usize>> = Vec::new();
        for floor in 0..=k {
            let mut people_rooms: Vec<usize> = Vec::new();
            for i in 1..=n {
                if floor == 0 {
                    people_rooms.push(i);
                } else {
                    people_rooms.push(people_floors[floor-1].iter().take(i).sum());
                }
            }

            people_floors.push(people_rooms);
        }

        writeln!(buf_write, "{}", people_floors[k][n-1]).ok();
    }

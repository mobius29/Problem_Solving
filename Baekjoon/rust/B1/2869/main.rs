...
fn factorial(arr: &mut [usize; 11], n: usize) -> usize {
    if n == 1 {
        1
    } else {
        arr[n] = n * factorial(arr, n - 1);
        arr[n]
    }
}
...
fn main() {
    ...
    let (a, b, v): (usize, usize, usize) = line.next();

    let x = v - a;
    let y = a - b;
    let answer = if x % y == 0 { x / y } else { (x / y) + 1 };
    writeln!(buf_write, "{}", answer + 1).ok();

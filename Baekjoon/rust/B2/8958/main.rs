...
fn main() {
    ...
    for _test_case in 0..t {
        let mut answer = 0;
        let ox_result: &str = line.next_str();

        let mut in_a_row_answer_count = 0;
        ox_result.chars().into_iter().for_each(|c| {
            if c == 'O' {
                in_a_row_answer_count += 1;
                answer += in_a_row_answer_count;
            }

            if c == 'X' {
                in_a_row_answer_count = 0;
            }
        });

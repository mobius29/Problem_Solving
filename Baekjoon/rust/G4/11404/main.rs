...
fn floyd_warshall(paths: &mut Vec2<usize>, n: usize) {
    for k in 1..=n {
        for i in 1..=n {
            for j in 1..=n {
                if i == j {
...
fn main() {
    ...
    for _ in 0..m {
        let (a, b, c): (usize, usize, usize) = line.next();
        paths[a][b] = min(paths[a][b], c);
    }

    floyd_warshall(&mut paths, n);

    for i in 1..=n {
        for j in 1..=n {
            let cost = if paths[i][j] == INF_USIZE {
                0
            } else {
                paths[i][j]
            };
            write!(buf_write, "{} ", cost).ok();
        }

...
fn main() {
    ...
    let count_list: Vec<Count> = (0..=40).into_iter().enumerate().fold(vec![], |mut acc, (idx, i)| {
        acc.push(match i {
            0 => Count { zero: 1, one: 0 },
            1 => Count { zero: 0, one: 1 },
            _ => Count { zero: acc[idx - 2].zero + acc[idx - 1].zero, one: acc[idx - 2].one + acc[idx - 1].one },
        });
        acc
    });
    ...
    for _ in 0..t {
        ...
        writeln!(buf_write, "{} {}", count_list[n].zero, count_list[n].one).ok();
    }

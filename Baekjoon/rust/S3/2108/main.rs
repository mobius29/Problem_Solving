...
fn main() {
    ...
    let mut nums: Vec<i32> = line.next_iter().take(n).collect();
    nums.sort();

    let sum: i32 = nums.iter().sum();
    let average = (sum as f32 / n as f32).round() as i32;
    let median = nums[n / 2];
    let range = nums[n - 1] - nums[0];

    let mut nums_have_max_count: Vec<i32> = Vec::new();
    ...
    for i in 1..n {
        ...
        if current_count > max_count {
            nums_have_max_count = vec![nums[i-1]];
            max_count = current_count;
        } else if current_count == max_count {
            nums_have_max_count.push(nums[i-1]);
        }
    ...
    if current_count > max_count {
        nums_have_max_count = vec![nums[n-1]];
    } else if current_count == max_count {
        nums_have_max_count.push(nums[n-1]);
    }

    let mode = if nums_have_max_count.len() == 1 { nums_have_max_count[0] } else { nums_have_max_count[1] };

    writeln!(buf_write, "{}\n{}\n{}\n{}", average, median, mode, range).ok();

...
fn solve(hashmap: &HashMap<String, Vec<String>>) -> usize {
    let list: Vec<usize> = hashmap.values().map(|v| v.len()).collect();
    let answer = list.iter().fold(1, |acc, x| acc * (x + 1)) - 1;
...
fn main() {
    ...
    for _ in 0..t {
        ...
        for _ in 0..n {
            let cloth = line.next_str().to_string();
            let kind = line.next_str().to_string();

            match hashmap.get_mut(&kind) {
                Some(v) => v.push(cloth),
                None => {
                    hashmap.insert(kind, vec![cloth]);
                }
            };
        }

        let answer = solve(&hashmap);
        writeln!(buf_write, "{}", answer).unwrap();
    }

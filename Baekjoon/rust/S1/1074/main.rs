...
fn solve(n: usize, r: usize, c: usize) -> usize {
    match n {
        0 => panic!(),
        1 => 2 * r + c,
        _ => {
            let k = 2usize.pow((n - 1) as u32);
            match (r, c) {
                (a, b) if a < k && b < k => solve(n - 1, a, b),
                (a, b) if a < k && b >= k => solve(n - 1, a, b % k) + (k * k),
                (a, b) if b < k => solve(n - 1, a % k, b) + (k * k) * 2,
                (a, b) => solve(n - 1, a % k, b % k) + (k * k) * 3,
            }
        }
    }
}
...
fn main() {
    ...
    let (n, r, c): (usize, usize, usize) = line.next();
    let answer = solve(n, r, c);

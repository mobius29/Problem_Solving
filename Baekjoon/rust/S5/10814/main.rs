...
fn main() {
    ...
    let mut subscribers: Vec<Person> = Vec::new();

    for _i in 0..n {
        let age: usize = line.next();
        let name: &str = line.next_str();

        subscribers.push(Person { age, name: name.to_string() });
    }

    subscribers.sort_by(|a, b| a.age.cmp(&b.age));

    for subscriber in subscribers.iter() {
        writeln!(buf_write, "{} {}", subscriber.age, subscriber.name).ok();
    }
    // flush buffer

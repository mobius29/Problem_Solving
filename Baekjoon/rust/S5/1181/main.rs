...
fn main() {
    ...
    let mut word_list: HashSet<&str> = HashSet::new();

    let n: usize = line.next();
    for _i in 0..n {
        let word: &str = line.next_str();
        if !word_list.contains(word) {
            word_list.insert(word);
        }
    }

    let mut word_list_array: Vec<&str> = word_list.into_iter().collect();
    word_list_array.sort_by(|a, b| {
        if a.len() == b.len() {
            return a.cmp(b);
        }
    ...
    for str in word_list_array {
        writeln!(buf_write, "{}", str).ok();
    }

...
fn main() {
    ...
    for _i in 0..n {
        let instruction: &str = line.next_str();
        ...
        if instruction == "pop" {
            ...
            writeln!(buf_write, "{}", top).ok();
        }

        if instruction =="size" {
            writeln!(buf_write, "{}", stack.size).ok();
        }

        if instruction == "empty" {
            let is_empty = if stack.empty() { 1 } else { 0 };
            writeln!(buf_write, "{}", is_empty).ok();
        }

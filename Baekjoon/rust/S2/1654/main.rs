...
fn main() {
    ...
    let (k, n): (u32, u32) = line.next();
    let lans: Vec<u32> = line.next_iter().take(k as usize).collect();

    let max_lan = *lans.iter().max().unwrap();

    let mut left = 1u32;
    let mut right = max_lan;
    ...
    while left <= right {
        ...
        let cutted_lans_count = lans.iter().fold(0u32, |acc, lan| acc + (lan / m));

        if cutted_lans_count < n {
            right = m - 1;
        } else {
            answer = m;
            left = m + 1;

...
fn solve(paper: &Vec<Vec<usize>>, start: (usize, usize), size: usize) -> (usize, usize) {
    let (x, y) = start;
    if size == 1 {
        return if paper[x][y] == 0 { (1, 0) } else { (0, 1) };
    }
    ...
    let quarter_papers: Vec<(usize, usize)> = d
        .iter()
        .map(|(dx, dy)| solve(paper, (x + *dx, y + *dy), half_size))
        .collect();

    if quarter_papers.iter().all(|(a, _)| *a == 0) {
        return (0, 1);
    }

    if quarter_papers.iter().all(|(_, b)| *b == 0) {
        return (1, 0);
    }
...
fn main() {
    ...
    let mut paper: Vec<Vec<usize>> = vec![];
    for _ in 0..n {
        let row: Vec<usize> = line.next_iter().take(n).collect();
        paper.push(row);
    }

    let (white, blue) = solve(&paper, (0, 0), n);
    writeln!(buf_write, "{}\n{}", white, blue).ok();

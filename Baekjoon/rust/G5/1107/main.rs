...
fn solve(n: usize, forbidden_buttons: [bool; 10]) -> usize {
    let mut answer: usize = 500_001;
    let mut deque: VecDeque<Channel> = VecDeque::new();
    forbidden_buttons
        .iter()
        .enumerate()
        .for_each(|(idx, is_forbidden)| {
            if !is_forbidden {
                deque.push_back(Channel {
                    current: idx,
                    count: 1,
                });
            }
    ...
    let mut visited = [false; 1_000_001];

    while !deque.is_empty() {
        let Channel { current, count } = deque.pop_front().unwrap();
        let added_count = current.abs_diff(n) + count;
        answer = min(answer, added_count);
        ...
        forbidden_buttons
            ...
            .for_each(|(idx, is_forbidden)| {
                if !is_forbidden {
                    let current = current * 10 + idx;
                    if current <= 1_000_000 && !visited[current] {
                        deque.push_back(Channel { current, count });
...
fn main() {
    ...
    let mut forbidden_buttons = [false; 10];
    line.next_iter()
        .take(m)
        .for_each(|button: usize| forbidden_buttons[button] = true);

    let answer = min(n.abs_diff(100usize), solve(n, forbidden_buttons));

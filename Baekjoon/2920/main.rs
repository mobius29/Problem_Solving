...
fn main() {
    ...
    let ascending = vec![1, 2, 3, 4, 5, 6, 7 ,8];
    let descending = vec![8, 7, 6, 5, 4, 3, 2, 1];
    ...
    let is_mixed = !is_ascending && !is_descending;

    if is_ascending {
        writeln!(buf_write, "ascending").ok();
    }

    if (is_descending) {
        writeln!(buf_write, "descending").ok();
    }

    if (is_mixed) {
        writeln!(buf_write, "mixed").ok();
    }

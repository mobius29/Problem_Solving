...
fn main() {
    ...
    let A: usize = line.next();
    let B: usize = line.next();
    let C: usize = line.next();

    let mut multiplied_result = A * B * C;
    let mut number_counts: Vec<u32> = vec![0; 10];

    while multiplied_result != 0 {
        let num: usize = multiplied_result % 10;
        number_counts[num] += 1;

        multiplied_result /= 10;
    }
    // write
    ...
    for i in 0..10 {
        writeln!(buf_write, "{}", number_counts[i]).ok();
    }

...
fn main() {
    ...
    let shifts = ['E', 'S', 'T', 'J'];
    let test_cases: usize = line.next();
    for _ in 0..test_cases {
        let n: usize = line.next();
        let mut mbtis: [usize; 16] = [0; 16];
        for _ in 0..n {
            let mbti = line.next_str().to_string();

            let mut index = 0usize;
            for (idx, c) in mbti.chars().enumerate().into_iter() {
                if c == shifts[idx] {
                    index ^= 1 << (3 - idx);
                }
            }
        ...
        for i in 0..16 {
            ...
            for j in 0..16 {
                ...
                for k in 0..16 {
                    ...
                    let diff = (i ^ j).count_ones() + (j ^ k).count_ones() + (k ^ i).count_ones();

...
fn main() {
    ...
    let splitted_by_minus: Vec<i32> = str.split('-').fold(vec![], |mut acc, x| {
        let item = x
            .split('+')
            .fold(0, |acc, value| acc + value.parse::<i32>().unwrap());
        acc.push(item);
        acc
    });

    let answer = splitted_by_minus[0] - splitted_by_minus[1..].iter().sum::<i32>();
